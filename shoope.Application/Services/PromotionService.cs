    ...
    public class PromotionService : IPromotionService
    {
        ...
        public async Task<ResultService<PromotionDTO>> Create(PromotionDTO? promotionDTO)
        {
            try
            {
                ...
                {
                    result = await _cloudinaryUti.CreateImg(promotionDTO.Img, "img-shopee", 360, 360);
                ...
                else if (promotionDTO.WhatIsThePromotion == 2)
                {
                    ...
                    {
                        resultCreateImgFirst = await _cloudinaryUti.CreateImg(promotionDTO.ImgInnerFirst, "img-shopee", 360, 360);
                    }

                    if(promotionDTO.ImgInnerSecond != null)
                    {
                        resultCreateImgSecond = await _cloudinaryUti.CreateImg(promotionDTO.ImgInnerSecond, "img-shopee", 360, 360);
                    }

                    if (promotionDTO.ImgInnerThird != null)
                    {
                        resultCreateImgThird = await _cloudinaryUti.CreateImg(promotionDTO.ImgInnerThird, "img-shopee", 360, 360);
                    }
                ...
                var createPromotion = await _promotionRepository.Create(Promotion);
        ...
        public async Task<ResultService<PromotionDTO>> DeletePromotion(Guid promotionId)
        {
            ...
            {
                await _unitOfWork.BeginTransaction();
                var deleteMovie = await _promotionRepository.Delete(promotionDelete);
